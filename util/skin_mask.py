...
def get_skin_mask(img_path):
    ...
    for root, dirs, files in os.walk(os.path.join(img_path, 'images')):
        if len(files) > 0 and not os.path.exists(root.replace('images', 'mask')):
    ...
    for i in range(0, len(names)):
        ...
        cv2.imwrite(os.path.join(save_path, imgName), skin_img.astype(np.uint8))
