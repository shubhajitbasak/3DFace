...
gmm_skin_cov_inv = [np.array([[0.0019472069, 0.0020450759, -0.00060243998], [0.0020450759, 0.017700525, 0.0051420014],
...
gmm_nonskin_cov_inv = [np.array(
    [[0.00085371657, 0.00071197288, 0.00023958916], [0.00071197288, 0.0025935620, 0.00076557708],
     [0.00023958916, 0.00076557708, 0.0015042332]]),
                       np.array(
                           [[0.00024650150, 0.00045542428, 0.00015019422], [0.00045542428, 0.026412144, 0.018419769],
                            [0.00015019422, 0.018419769, 0.037497383]]),
                       np.array(
                           [[0.00037054974, 0.00038146760, 0.00040408765], [0.00038146760, 0.0085505722, 0.0079136286],
...
def get_skin_mask(img_path):
    ...
    for i in range(0, len(names)):
        ...
        if not os.path.exists(os.path.split(full_image_name)[0].replace('images', 'mask')):
            os.makedirs(os.path.split(full_image_name)[0].replace('images', 'mask'), exist_ok=True)

        cv2.imwrite(full_image_name.replace('images', 'mask'), skin_img.astype(np.uint8))
