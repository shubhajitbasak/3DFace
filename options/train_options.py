...
class TrainOptions(BaseOptions):
    ...
    def initialize(self, parser):
        ...
        # dataset parameters
        # for train
        parser.add_argument('--name', type=str, default='face_recon', help='name of the experiment. It decides where '
        ...
        parser.add_argument('--max_dataset_size', type=int, default=float("inf"), help='Maximum number of samples '
                                                                                       'allowed per dataset. If the '
                                                                                       'dataset directory contains '
                                                                                       'more than max_dataset_size, '
                                                                                       'only a subset is loaded.')
        parser.add_argument('--preprocess', type=str, default='shift_scale_rot_flip', help='scaling and cropping of '
                                                                                           'images at load time ['
                                                                                           'shift_scale_rot_flip | '
                                                                                           'shift_scale | shift | '
                                                                                           'shift_rot_flip ]')
        parser.add_argument('--use_aug', type=util.str2bool, nargs='?', const=True, default=True, help='whether use '
                                                                                                       'data '
                                                                                                       'augmentation')
